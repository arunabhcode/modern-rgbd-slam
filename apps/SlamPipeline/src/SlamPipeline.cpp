...
int main(int argc, char* argv[])
{
  ...
  room::Feature feature_inst(show_debug,
                             ...
                             fast_threshold,
                             num_points);
  // feature_inst.FeatureMatch(frame0, frame1, false, false, threshold_factor);
  std::vector<int> match_idx;
  feature_inst.FeatureTrack(frame0, frame1, cv::Size(21, 21), 3, match_idx);

  room::Odometry odometry_inst(show_debug);
  room::Pose out_pose =
      odometry_inst.PoseEstimationICP(frame0, frame1, match_idx);
