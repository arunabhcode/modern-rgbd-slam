...
#include "RGBDSlam/BundleAdjustment.h"
                               ...
                               Vertex* v_frame)
{
  ...
  edge_se3->setMeasurement(Eigen::Vector2d(frame.m_keypoints[kp_idx].x,
                                           frame.m_keypoints[kp_idx].y));
                                  ...
                                  std::vector<std::shared_ptr<Frame>> frames)
{
  ...
  {
    AddVertex(*mps[j]);
    for (std::map<int, int>::const_iterator ob_it =
             mps[j]->m_observations.begin();
      ...
      AddEdge(ob_it->second,
              *frames[ob_it->first],
              dynamic_cast<g2o::OptimizableGraph::Vertex*>(m_optimizer.vertex(
                  frames.size() + j +
                  1)),  // +1 because both frame count and j start from 0
              dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                  m_optimizer.vertex(ob_it->first)));
    }
  }
}
